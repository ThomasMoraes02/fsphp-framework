...
class App extends Controller
{
    ...
    public function wallets(?array $data): void
    {
        ...
        if(!empty($data['wallet']) && !empty($data['wallet_name'])) {
            // Premium resource
            $subscribe = (new AppSubscription)->find("user_id = :user AND status != :status", "user={$this->user->id}&status=canceled")->fetch();
            if(!$subscribe->count()) {
        ...
        $wallets = (new AppWallet)->find("user_id = :user", "user={$this->user->id}")
        ->order("wallet")
        ->fetch(true);
    ...
    public function launch(array $data): void
    {
        ...
        if(!$wallet->free && !$subscribe->count()) {
            $this->message->error("Sua carteira {$wallet->wallet} é PRO {$this->user->first_name}. Para controla-la, precisa ser PRO.")->flash();
            echo json_encode(["redirect" => url("/app/assinatura")]);
            return;
        }
        ...
        $invoice->enrollments = intval((!empty($data['enrollments']) ? $data['enrollments'] : 1));
    ...
    public function support(array $data): void
    {
        if(empty($data['message'])) {
            $json['message'] = $this->message->warning("Para enviar, escreve sua mensagem")->render();
            echo json_encode($json);
            return;
        }
    ...
    public function onpaid(array $data): void
    {
        ...
        if(!$invoice) {
            $this->message->error("Ooops! Ocorreu um erro ao atualizar lançamento :/")->flash();
            $json['reload'] = true;
            echo json_encode($json);
            return;
        }
    ...
    public function invoice(array $data): void
    {
        if(!empty($data['update'])) {
            ...
            if($data['due_day'] < 1 || $data['due_day'] > $dayOfMonth = date("t", strtotime($invoice->due_at))) {
                $json['message'] = $this->message->warning("O vencimento deve ser entre o dia 1 e dia {$dayOfMonth} para este mês.")->render();
                echo json_encode($json);
                return;
            }
