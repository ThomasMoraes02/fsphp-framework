...
class Pay extends Controller
{
    ...
    public function create(array $data): void
    {
        ...
        if(request_limit('pay_create', 3, 60*5)) {
        ...
        $checkSubscribe = (new AppSubscription)->findById("user_id = :user AND status != :status", "user={$user->id}&status=canceled")->fetch();

        if($checkSubscribe) {
            $json['message'] = $this->message->warning("Você já tem uma assinatura ativa {$user->first_name}. Não é necessário assinar o {$plan->name} mais uma vez.")->render();
            echo json_encode($json);
            return;
        }
        ...
        if(!$transaction) {
            $json['message'] = $card->message()->before('Ooops! ')->after('. Você pode tentar novamente com um novo cartão.')->render();
            echo json_encode($json);
            return;
        }
        ...
        $this->message->success("Bem-vindo(a) ao {$plan->name} {$user->first_name}. Sua assinatura está ativa e você já pode controlar. Confira os detalhe...")->flash();
    ...
    {
        $user = Auth::user();
        $subscribe = (new AppSubscription)->findById("user_id = :user AND status != :status", "user={$user->id}&status=canceled")->fetch();
